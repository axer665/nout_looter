...
class objectParam extends React.Component {
    ...
    editOk = () => {
        ...
        const headers = ApiUser.getHeaders()

        Axios.put('http://192.168.2.119:84/api/updateUser/'+this.props.user.id, data, {
            headers: headers
        })
            .then((response) => {
    ...
    render(){
        ...
        return(
            ...
            <div className="d-flex flex-row bd-highlight mb-3 justify-content-center container-parameters-item-line ">
                <div className="user_data-settings-header">
                    {this.state.keyName}
                </div>
                <div className="user_data-settings-value">
                    {this.state.value}
                </div>
                <div className="user_data-settings-control">
                    {this.state.control}
                </div>
            </div>
        )
