...
class settingsProjectType extends React.Component {
    ...
    editOk = () => {
        ...
        Axios.put('http://192.168.2.119:84/api/updateSettingsProjectType/'+this.state.type.id, data, {
            headers: headers
        })
        .then((response) => {
    ...
    deleteType = () => {
        ...
        Axios.delete('http://192.168.2.119:84/api/settingsProjectType/'+data.id, {headers : headers, data : data})
        .then((response) => {
            console.log(response.data)
    ...
    render(){
        return (
            <div className="d-flex flex-row bd-highlight mb-3 container-list-settings_project_types justify-content-center">
                ...
                <div className="s_project_types_list-item-control">
                    { this.state.control }
                </div>
            </div>
        )
